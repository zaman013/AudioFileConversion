...
namespace Xano.AudioFileConversion
{
	public static class WaveResampler
	{
		...
		public static void Resample(string filePathInput, string filePathOutput, int resampleSize)
		{
			...
			using (var reader = new WaveFileReader(filePathInput))
			{
				...
				using (var resampler = new MediaFoundationResampler(reader, outFormat))
				{
					...
					stream.Position = 0;
					(sampleRate, waveFileHeader, waveFileData) = ReadStream(stream);
				}
			}

			var (phraseData, simulatedData) = ProcessRawVoiceData(waveFileData);

			// TODO: Figure out how to write to a wave file. 
			// recreate wave file using header we saved from original wave file.
			var result = new byte[simulatedData.Length * sizeof(short)];
			Buffer.BlockCopy(simulatedData, 0, result, 0, result.Length);
			var completeWaveFile = waveFileHeader.Concat(result).ToArray();
			var fileName = @"C:\Users\uzzamana\Documents\MATLAB\sample_out\testFinalFile.wav";
			WaveFormat waveFormat = new WaveFormat(20000, 1);
			using (WaveFileWriter writer = new WaveFileWriter(fileName, waveFormat))
			{
				writer.Write(completeWaveFile, 0, completeWaveFile.Length);
			}
		...
		private static (int sampleRate, byte[] waveFileHeader, double[] rawWaveData) ReadStream(Stream fs)
		{
